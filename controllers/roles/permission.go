...
func (this *ManagePermissionController) Get() {
	...
	countPermission := roles.CountPermission(condArr)

	paginator := pagination.SetPaginator(this.Ctx, offset, countPermission)
	_, _, permissions := roles.ListPermission(condArr, page, offset)
	...
	condArrP["parentid"] = "0"
	_, _, parentpermissions := roles.ListParentPermission()
	this.Data["parentpermissions"] = parentpermissions

	this.TplName = "roles/permission.tpl"
}
...
func (this *FormPermissionController) Get() {
	...
	if "" != idstr {
		...
		permission, _ = roles.GetPermission(int64(id))
	} else {
		//权限检测
	...
	condArr := make(map[string]string)
	condArr["parentid"] = "0"
	_, _, permissions := roles.ListPermission(condArr, 0, 100)
	this.Data["permissions"] = permissions

	this.TplName = "roles/permission-form.tpl"
}
...
func (this *FormPermissionController) Post() {
	...
	permission.Url = ename
	permission.IsShow = ptype
	permission.Sort = weight
	...
	if permissionid <= 0 {
		...
		err = roles.AddPermission(permission)
	} else {
		err = roles.UpdatePermission(permissionid, permission)
	}
...
func (this *AjaxDeletePermissionController) Post() {
	...
	err := roles.DeletePermission(ids)
